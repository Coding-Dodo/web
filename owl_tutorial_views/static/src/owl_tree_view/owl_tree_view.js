...
class OWLTreeView extends owl.Component {
    ...
    async _onTreeItemClicked(ev) {
        ...
        if (ev.detail.data.children === undefined) {
            await this.model.expandChildrenOf(ev.detail.data.id, ev.detail.data.parent_path);
        } else {
    ...
    async _onChangeItemTree(ev) {
        ev.stopPropagation();
        const itemMoved = ev.detail.itemMoved;
        const newParent = ev.detail.newParent;
        await this.model.changeParent(itemMoved.id, newParent.id);

        // Refresh old parent
        const oldParent = await this.model.refreshNode(itemMoved.parent_id[0]);
        await this.model.expandChildrenOf(oldParent.id, oldParent.parent_path);

        // Refresh new parent
        await this.model.expandChildrenOf(ev.detail.newParent.id, ev.detail.newParent.parent_path);
    }
}
